...
public class DoctorRegistrationActivity extends AppCompatActivity {
    ...
    private EditText regfullname,regidn,regphone,logemail,logpassword,speciality;
    private Button regButton;
    private CircleImageView profileImage;

    private ProgressBar progressbar;
    String reg_fname, reg_mname, reg_lname, reg_mobnum, docreg_cnic, reg_addr, reg_password, reg_gender, reg_dob,spec;
 ...
 private FirebaseAuth mAuth;
  ...
  //  private CollectionReference docsRef = db.collection("Users").document("Doctor").collection("Doctors");
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        speciality=findViewById(R.id.spec1);
        ...
        regButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
               ...
               final String email=logemail.getText().toString().trim();
                ...
                final String speci = speciality.getText().toString().trim();
                    ...
                    mAuth.createUserWithEmailAndPassword(email,password).addOnCompleteListener(new OnCompleteListener<AuthResult>() {
                        ...
                        public void onComplete(@NonNull Task<AuthResult> task) {
                            if(!task.isSuccessful()){
                                ...
                                userInfo.put("firstname",fullname);
                                userInfo.put("demail",email);
                                userInfo.put("did",idnumber);
                                userInfo.put("contact",phone);
                                userInfo.put("dpassword",password);
                                userInfo.put("speciality",speci);
                                userDatabaseRef.updateChildren(userInfo).addOnCompleteListener(new OnCompleteListener() {
                                    @Override
                                    public void onComplete(@NonNull Task task) {
                                        if(task.isSuccessful()){
                                            Toast.makeText(DoctorRegistrationActivity.this, "Successfull", Toast.LENGTH_SHORT).show();
                                        }
                                        else{
                                ...
                                docreg_cnic= idnumber.toString().trim();

                                db.collection("Users").document("Doctor").collection("Doctors").document(docreg_cnic).set(userInfo)
                              //  db.collection("Users").document("Doctor").set(userInfo)
                                                    ...
                                                    public void onSuccess(Uri uri) {
                                                        ...
                                                        userDatabaseRef.updateChildren(newImageMap).
                                                                addOnCompleteListener(new OnCompleteListener() {
                                                                    ...
                                                                    public void onComplete(@NonNull Task task) {
                                                                        if(task.isSuccessful()){
                                                                            Toast.makeText(DoctorRegistrationActivity.this, "Reg successfull", Toast.LENGTH_SHORT).show();
                                                                        }
                                                                        else{
                                                                            Toast.makeText(DoctorRegistrationActivity.this, task.getException().toString(), Toast.LENGTH_SHORT).show();
                                                                        }
